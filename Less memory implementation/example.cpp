...
int Example_Fixed_Length_Memory_Pool(void)
{
	...
	FixLen_MemPool<void> pPool(1024, 16);

	//分配时返回long*
	FixLen_MemPool<long> longPool;
	//默认构造函数不会分配内存，需要手动调用分配器
	longPool.Realloc();
	...
	size_t szAlignedSize = FixLen_MemPool<Test>::Aligned(8, sizeof(Test));//使用了静态函数Aligned计算内存对齐
	FixLen_MemPool<Test, 8> testPool(1024, szAlignedSize);//这样实际分配的内存是8字节，使用1字节，剩余7字节用于对齐，同时模板指定第一个内存块的起始地址是对齐8字节边界的
